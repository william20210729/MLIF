...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc = nn.Linear(config.hidden_size*(self.layercount+self.pad_size), config.num_classes) ### 768, 5  修改第一个参数
    ...
    def forward(self, x):
        ...
        att = self.attention(d.to(
            self.device))  ## RuntimeError: mat1 and mat2 shapes cannot be multiplied (2x9216 and 768x768)

        a = att
        b = a.view(context.shape[0] * (self.pad_size+self.layercount) * 768)  ##############
        c = b.reshape(context.shape[0], (self.pad_size+self.layercount)  * 768)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = attention(
            query.to(self.device), key.to(self.device), value.to(self.device)
        )
